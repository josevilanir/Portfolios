...
const MacOSDock: React.FC<MacOSDockProps> = ({
  ...
  const [config, setConfig] = useState<{ baseIconSize: number; maxScale: number; effectWidth: number }>(
  ...
  const handleAppClick = (appId: string, index: number) => {
    if (iconRefs.current[index]) {
      if (typeof window !== 'undefined' && (window as Window & { gsap?: unknown }).gsap) {
        const gsap = ((window as unknown) as { gsap: { to: (el: HTMLElement | null, opts: object) => void } }).gsap;
